---
name: implementer
description: Execute a plan already written in .plan/ test-first (inverse TDD) in this Python Secret Santa (Amigo Invisible) CLI app, marking subtasks done. Use when the user says "implementa el plan", "ejecuta el plan", "desarrolla esto", "implementer".
---
...
6. **Done**: with the final gate green (full suite + `ruff check .`, `dag-execution.md` §Walk), set the plan's
...
- `../references/common-commands.md` — run/test commands (`pytest`, `ruff`, `python main.py`).

Load on demand only when needed:
- `../references/common-workflows.md` — when the plan matches a recipe (add a YAML config key, add a
  secret, change the assignment/restriction rules, change the email template, add a CLI flag, bump
  version).
