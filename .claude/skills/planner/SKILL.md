---
name: planner
description: Create a development plan for a feature/fix/refactor in this Python Secret Santa (Amigo Invisible) CLI app. Classifies the usecase, maps the goal to concrete files/tests, and writes the plan to disk at .plan/<usecase>/<type>/<slug>.md. Writes the plan and STOPS (does not implement). Use when the user says "planifica", "crea un plan", "nueva feature", "planner".
---
...
- **usecase** ∈ `assignment` | `email` | `config` | `general` (cross-cutting: packaging, CI, docs,
  tooling, versioning, project layout, `utils/`).
- **type** ∈ `feat` | `fix` | `refactor`.
- **slug** kebab-case, ≤5 words (`add-gift-budget-field`, `project-reorg-ruff`).
...
- `../references/common-workflows.md` — recipes for the common changes (add a YAML key, add a secret,
  change the assignment rules, change the email template, add a CLI flag, bump version).
